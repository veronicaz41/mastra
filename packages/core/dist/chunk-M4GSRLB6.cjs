...
exports.Agent = class Agent extends (_a = chunkD63P5O4Q_cjs.MastraBase) {
  ...
  async generateTitleFromUserMessage({
    ...
    const {
      ...
      container: new chunkLZQIMLJV_cjs.Container(),
      messages: [{
        role: "system",
        content: `
...
`)
  ...
  async generate(messages, generateOptions = {}) {
    ...
    const normalizedContainer = container ?? new chunkLZQIMLJV_cjs.Container();
    const {
      before,
      after
    } = this.__primitive({
...
var WorkflowInstance = class {
  ...
  async runMachine(parentStepId, input, container = new chunkLZQIMLJV_cjs.Container()) {
  ...
  async resumeWithEvent(eventName, data, container = new chunkLZQIMLJV_cjs.Container()) {
